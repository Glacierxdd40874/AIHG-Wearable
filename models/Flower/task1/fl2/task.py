...
def load_model():
    model = Sequential([
        ...
        Dense(1, activation='linear')  # Predicting SleepEfficiency
    ])
...
def load_data(partition_id, num_partitions):
    ...
    if data is None:
        data_path = r"E:\5703\Week11\t1.csv"
        df = pd.read_csv(data_path)


        df = df.dropna(subset=["SleepEfficiency"])

        
        y = df["SleepEfficiency"].values.reshape(-1, 1)  # shape (n_samples, 1)
        
        if 'BMI' in df.columns:
            df = df.drop(columns=['BMI'])

        
        X = df.drop(columns=["SleepEfficiency"]).values  
        ...
        np.save(r"E:\5703\Week11\t1.npy", mean)
        np.save(r"E:\5703\Week11\t1.std", std)
